...
docker tag ros2:amd64-$ROS_DISTRO $DOCKER_USERNAME/ros2:amd64-$ROS_DISTRO-$TIMESTAMP
docker tag ros2:arm64-$ROS_DISTRO $DOCKER_USERNAME/ros2:arm64-$ROS_DISTRO-$TIMESTAMP
docker images
if [[ "$IS_PULL_REQUEST" == "false" ]] && [[ "$GIT_BRANCH" == "master" ]]; then
	...
	docker push $DOCKER_USERNAME/ros2:arm64-$ROS_DISTRO-$TIMESTAMP
	docker push $DOCKER_USERNAME/ros2:amd64-$ROS_DISTRO-$TIMESTAMP
